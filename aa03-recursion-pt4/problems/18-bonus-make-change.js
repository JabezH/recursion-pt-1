...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  // Your code here 
  // Base case: if the target amount is 0, return an empty array (no coins needed)
  if (target === 0) {
    return [];
}
...
coins.forEach(coin => {
    // Check if the coin is smaller than or equal to the target amount
    if (coin <= target) {
        // Recursively call makeBetterChange on the remainder using coins less than or equal to the current coin
        const change = makeBetterChange(target - coin, coins.filter(c => c <= coin));

        // If change is not null (i.e., a valid solution exists), add the current coin to the change
        if (change !== null) {
            change.push(coin);

            // Update the bestChange if it's null or the new change is better (i.e., has fewer coins)
